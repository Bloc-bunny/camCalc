...
- (TraccsVialHistory *) findByQuarterName:(NSString *)qtrname {
    ...
    for (int i=0; i<[DataModel sharedInstance].vialHistory.count; i++) {
        item = [[DataModel sharedInstance].vialHistory objectAtIndex:i];
...
- (void) drawChart:(NSMutableArray *)xArray {
    ...
    for (int index=0; index<quartersArray.count; index++) {
        ...
        if (item.state == 0) {
            ...
            NSLog(@"Start new path: %f, %f", lastPoint.x, lastPoint.y);
