...
- (void) update {
    ...
    while ([rs next]) {
        ...
        if (dict != nil) {
            ...
            [vialHistory addObject:vialRecord];
    ...
    NSLog(@"Max value is %i in vial history with %i items", maxQty, vialHistory.count);
    [DataModel sharedInstance].trendMax = maxQty;
    [DataModel sharedInstance].vialHistory = vialHistory;
